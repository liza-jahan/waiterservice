...
const paragraphs = document.querySelectorAll('.slider_section p');


function animateParagraphsOnScroll() {
...
window.addEventListener('scroll', animateParagraphsOnScroll);
window.addEventListener('load', animateParagraphsOnScroll);
...
function showSlides() {
    ...
    currentIndex++;
    if (currentIndex > slides.length) currentIndex = 1; // Reset to first slide

    // Show the current slide
    slides[currentIndex - 1].style.display = 'block';
    slides[currentIndex - 1].classList.add('fade'); // Add fade animation
...
const observerCallback = (entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            entry.target.classList.add('show'); // Add the "show" class for animation
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
    const observer = new IntersectionObserver(
        ...
        {
            threshold: 0.1, // 10% of the element must be visible for the animation to trigger
        }
    );
